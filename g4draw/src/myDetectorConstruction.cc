...
//构造材料，方法一
	...
	G4Material* pBerylium = new G4Material("Berylium",1.84*g/cm3,1,kStateSolid);
	//填充材料，方法一
...
//Air = man->FindOrBuildMaterial("G4_AIR");
	...
	//world
  	pSolidWorld= new G4Box("solidWorld",0.5*60*cm,0.5*60*cm,0.5*60*cm);
	...
	//target
	G4Box*             pSolidTarget = new G4Box("solidTarget",0.5*5*cm,0.5*5*cm,0.5*2.*um);
	G4LogicalVolume*   pLogicTarget = new G4LogicalVolume(pSolidTarget,pBerylium,"logicTarget",0,0,0,true);
	G4VPhysicalVolume* pPhysiTarget = new G4PVPlacement(0,G4ThreeVector(0,0,0),pLogicTarget,"phyciTarget",pLogicWorld,false,0);
	//SSD
	G4RotationMatrix *rot=new G4RotationMatrix();
	rot->rotateY(-20*deg);	//迎着转动轴方向，顺时针为正,与3D相反
	G4ThreeVector tran(10.*cm*sin(20*deg),0.,10.*cm*cos(20*deg));//translation
	//G4Box *pSolidBox= new G4Box("solidBox",0.5*10.*cm,0.5*10.*cm,0.5*1000.*um);
	pSolidSSD = new G4Box("solidSSD",0.5*5.*cm,0.5*5.*cm,0.5*20.*um);
